...
generate_relational_format <- function(cascades,filename,header_names){
  mat = summary(cascades)
  mat = mat[order(mat[, 1], mat[, 3], decreasing = FALSE), ]
  
  mat = data.frame(X = mat[, 2],
                   Y = mat[, 1],
                   Z = mat[, 3])
  
  names(mat) = header_names
...
generate_two_hops <- function(g,file_hops,negative_factor){
  ...
  write.table(POS,file_hops,col.names = c("Src","Dst","Class"),row.names = FALSE,sep = "\t")
  ...
  write.table(NEG,file_hops,col.names = F,row.names = FALSE,sep = "\t", append = TRUE,quote=FALSE)
}
